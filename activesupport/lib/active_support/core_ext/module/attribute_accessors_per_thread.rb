...
class Module
  ...
  def thread_mattr_reader(*syms, instance_accessor: true, instance_reader: true) # :nodoc:
    syms.each do |sym|
  ...
  def thread_mattr_writer(*syms, instance_accessor: true, instance_writer: true) # :nodoc:
    syms.each do |sym|
      raise NameError.new("invalid attribute name: #{sym}") unless /^[_A-Za-z]\w*$/.match?(sym)
